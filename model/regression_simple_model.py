...
def build_model(is_training, inputs, params):
    ...
    channels = [num_channels, num_channels * 2, num_channels * 4, num_channels * 8, num_channels * 4, num_channels * 2, num_channels, 2]
...
def model_fn(mode, inputs, params, reuse=False):
    ...
    assert labels.get_shape().as_list() == [None, params.image_size, params.image_size, 2]
    # -----------------------------------------------------------
    ...
    assert tf.norm(flat_labels - flat_logits, axis = 1,keepdims=True).get_shape().as_list() ==[None, 1]

    loss = tf.reduce_mean(tf.norm((flat_labels - flat_logits), axis = 1, keepdims=True))
