...
if args.toy:
	train_L = train_L[0:1]
	train_ab = train_ab[0:1]
	train_bins = train_bins[0:1]
	train_grayRGB = train_grayRGB[0:1]
...
# Show result
def showBestResult(dev_L, dev_bins, dev_ab, save_path):
	plt.figure()
...
def show5Results(dev_L, dev_bins, dev_ab, start_index, save_path):
	plt.figure()
	predict_bins, predict_ab, predict_costs = train_evaluate.predict(dev_L[start_index:start_index + 5], dev_bins[start_index:start_index + 5], dev_ab[start_index:start_index + 5], save_path)
	count = 0
	for i in range(5):
	    count = count + 1
...
def show1Result(dev_L, dev_bins, dev_ab, start_index, save_path):
	plt.figure()
...
if args.predict:
	...
	# showResult(dev_L, dev_bins, dev_ab, 20, save_path)
	# show5Results(dev_L, dev_bins, dev_ab, 10, best_path)
	show5Results(train_L, train_bins, train_ab, 10, last_path)
	# show1Result(train_L, train_bins, train_ab, 10, save_path)
