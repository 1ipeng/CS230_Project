...
def input_fn(is_training, data_L, data_ab, params):
	data_L = data_L / 100.
	data_ab = (data_ab + 128) / 255.
	num_samples = data_L.shape[0]

	if is_training:
		dataset = (tf.data.Dataset.from_tensor_slices((tf.constant(data_L), tf.constant(data_ab)))
